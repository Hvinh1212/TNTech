...
const ItemDetails = () => {
  ...
  const { products, fetchCartData, customerId, fetchCartUser } = useContext(DataContexts)
      ...
      <div className="w-11/12 h-auto flex flex-col py-5 mb-20  items-center place-self-center">
        <div className="w-11/12 px-24 flex ">
          <div className="w-full flex flex-col lg:flex-row justify-between gap-0">
            <div className="flex flex-col max-h-[500px] gap-2 items-center">
              {/* Căn giữa các phần tử */}
              <div className="flex justify-center mb-2 w-full">
                {/* Căn giữa ảnh chính và đảm bảo chiều rộng đầy đủ */}
                <img
                  src={activeImg}
                  alt="mainImage"
                  className="w-full h-auto max-w-[600px] max-h-[400px] object-cover border-b-2 rounded-lg"
                />
              </div>
              <div className="flex gap-2 overflow-x-auto flex-nowrap justify-center">
                {images.slice(1).map((image, index) => (
                  <img
                    key={index}
                    src={image}
                    alt=""
                    className="rounded-lg transition-opacity duration-200 w-[99px] h-[66px] object-cover cursor-pointer"
                    onMouseOver={() => setActiveImage(image)} // Cập nhật ảnh chính khi hover
                    onClick={() => {
            ...
            <div className="basis-5/12">
              <div className="w-full pb-5 mb-5 border-b-2 border-gray-500">
                <p className="text-2xl sm:text-3xl md:text-4xl font-bold mt-2 mb-1">{productName}</p>
                <p className="pt-5 text-sm sm:text-base md:text-lg font-bold flex flex-row items-center">
                  Hãng công nghệ:
                ...
                <p className="text-sm sm:text-base md:text-lg font-bold flex flex-row items-center">
                  Thể loại:
              ...
              <div className="w-full border-b-2 border-gray-500 mb-5 pb-5">
                {discount !== 0 ? (
                  <div className="w-full flex flex-row items-end gap-3">
                    ...
                    <p className="text-2xl sm:text-3xl md:text-4xl font-medium text-gray-500 line-through">
                      {formatNumber(price)}đ
                    </p>
                  </div>
                ) : (
                  <div className="w-full flex flex-row items-end gap-3">
                    <p className="font-bold text-2xl sm:text-3xl md:text-4xl">{formatNumber(price)}đ</p>
                  </div>
                )}
              </div>

              <div className="w-full">
                <p className="text-sm sm:text-base md:text-lg font-bold text-left my-2">Mô tả sản phẩm:</p>
              ...
              <div className="flex flex-row mt-5 gap-3">
                <button className="group overflow-hidden w-full sm:w-1/3 h-[60px] flex items-center justify-center border hover:bg-slate-700 border-[#3e3e3e] rounded-xl cursor-pointer group hover:border-none">
                  <p
                ...
                <button
                  ...
                  className="group overflow-hidden w-full sm:w-1/3 h-[60px] flex items-center justify-center border hover:bg-slate-700 border-[#3e3e3e] rounded-xl cursor-pointer group hover:border-none"
                >
