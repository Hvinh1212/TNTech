...
const Orders = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:5000/api/orders')
      ...
      .then(data => {
        const unconfirmedOrders = data.filter(order => order.order_is_paid === true);
  ...
  const handleEdit = (order) => {
    const updatedOrder = { ...order, order_is_paid: true };

    fetch(`http://localhost:5000/api/orders/${order.order_id}/pay`, {
      method: 'PUT',
            ...
            {orders.map(order => (
              <tr key={order.order_id}>
                ...
                <td>
                  ...
                  <button id="deleteBtn" onClick={() => handleDelete(order.order_id)}>Xoá</button>
                </td>
                <td>
